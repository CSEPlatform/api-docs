...
- CSE Wire Protocol (CSE)
- Light CSE Subprotocol (LES/2)
...
```
dpt.bootstrap(bootnode).catch((err) => console.error('SomCSEing went wrong!'))
```
...
- [Node discovery protocol](https://github.com/cse/wiki/wiki/Node-discovery-protocol)
- [RLPx - Node Discovery Protocol](https://github.com/cse/devp2p/blob/master/rlpx.md#node-discovery)
- [Kademlia Peer Selection](https://github.com/cse/wiki/wiki/Kademlia-Peer-Selection)
...
- `options.clientId` - Client ID string (default example: ``cse-devp2p/v2.1.3/darwin-x64/nodejs``).
- `options.remoteClientIdFilter` - Optional list of client ID filter strings (e.g. `['go1.5', 'quorum']`).
- `options.capabilities` - Upper layer protocol capabilities, e.g. `[devp2p.CSE.cse63, devp2p.CSE.cse62]`.
...
Upper layer protocol for exchanging CSE network data like block headers or transactions with a node, see [./src/cse/](./src/cse/).
...
```
CSE.on('message', async (code, payload) => {
  if (code === devp2p.CSE.MESSAGE_CODES.NEW_BLOCK_HASHES) {
    // Do somCSEing with your new block hashes :-)
  }
})
```
...
les.on('message', async (code, payload) => {
  if (code === devp2p.LES.MESSAGE_CODES.BLOCK_HEADERS) {
    // Do somCSEing with your new block headers :-)
  }
})
```
...
```
Add peer: 52.3.158.184:30303 GCSE/v1.7.3-unstable-479aa61f/linux-amd64/go1.9 (cse63) (total: 2)
...
- [pydevp2p](https://github.com/cse/pydevp2p) (Python)
- [Go cse](https://github.com/cse/go-cse/tree/master/p2p) (Go)
- [Exthereum](https://github.com/exthereum/exth_crypto) (Elixir)

### Links

- [Blog article series](https://ocalog.com/post/10/)  on implementing cse protocol stack
